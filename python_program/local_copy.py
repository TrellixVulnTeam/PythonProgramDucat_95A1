...
class ChatBox:
    ...
    def server_frame(self, name):
        ...
        # Button(f_server, text='Sign out', fg='blue', command=sign_out).place(x=230, y=25)
        Button(f_server, text="Create Server", command=make_server).place(x=100, y=170)
    ...
    def client_frame(self):
        ...
        Button(f_client, text="Home", fg='blue', command=go_home).place(x=245, y=5)
        Button(f_client, text="Start Talking", command=connect_to_server).place(x=100, y=140)

    def dashboard(self, name):

        def sign_out():
            f_dashboard.destroy()
            print("destroy dashboard")
            start.index()
        self.is_not_used()
        print("Now in dashboard home")
        global cur
        global con
        con = connect(db='tkinter', user='root', password='', host='localhost')
        cur = con.cursor()
        ...
        print(login_person_name)
        f_dashboard = Tk()
        # f_dashboard.geometry("600x300")
        f_dashboard.maxsize(width=600, height=300)
        f_dashboard.minsize(width=600, height=300)
        f_dashboard.title("Dashboard")

        txt_welcome = Label(f_dashboard, text="Welcome : ", font=("bold", 8))
        txt_welcome.place(x=470, y=20)

        login_name = Label(f_dashboard, text=login_person_name, font=("bold", 8))
        login_name.place(x=530, y=20)

        Button(f_dashboard, text='Sign out', fg='blue', command=sign_out).place(x=400, y=20)

        Button(f_dashboard, text='Product', height=3, width=10).place(x=200, y=100)

        Button(f_dashboard, text='Customer', height=3, width=10).place(x=300, y=100)
    ...
    def login(self):
        ...
        lb = Label(f_login, text="Login Form", font=("bold", 15))
        lb.place(x=100, y=20)
